/**
* Renders a role member relationship.
*/
RBAC.roleMember = function() {
  ...
  var memberLength = ( last.distanceTo(current) - RBAC.scale * RBAC.dimensions.role.radius * 2 );
  ...
  }
  {
    ...
    var pointerGeometry = new THREE.ConeGeometry(RBAC.scale * RBAC.dimensions.member_arrow.radius, 
      ...
      RBAC.runtime.scene.add(pointer);
    }
  }
  ...
  * @param {object} options - member: (true|false); whether to draw a member relationship.
  *   roleType: (person); icon to use on the role.
  */
  RBAC.role = function(roleName, options = {}) {
    ...
    {
      var role = new THREE.Mesh( RBAC.roleGeometry, RBAC.roleMaterial(options.roleType) );        
